...
class NewStudentReportController extends Controller
{

  public function ImportNewExcel(Request $request)
  {
      ...
      foreach ($request->file() as $f) {
        ...
        foreach ($items as $i) {
            ...
            if ($validator->fails()) {
              ...
              $honor = NewStudentReport::where('academic_year',$i->$column1)
              ...
              if(empty($honor)) {
                ...
                $honor->no_of_new_student_plan = $i->$column6;
                $honor->no_of_new_student = $i->$column7;
                ...
                try {
                  $honor = DB::select("UPDATE new_student_report
                    SET no_of_new_student_plan = ".$i->$column6."
                    , no_of_new_student = ".$i->$column7."
                    , updated_by = '".$user."'
                    , updated_dttm = now()
  ...
  public function ExportNewExcel(Request $request)
  {
      // select ข้อมูลที่ต้องการ export
      $query = "SELECT *
        FROM new_student_report
        WHERE 1 = 1";
      ...
      // ระบุชื่อไฟล์
      $filename = "import_new_student_template";
  ...
  public function ImportNewPercentExcel(Request $request)
  {
      $user = $request->input('user');
        ...
        foreach ($items as $i) {
            ...
            if ($validator->fails()) {
              ...
              if(empty($honor)) {
                ...
                $honor->no_of_new_student_plan = $i->$column6;
                $honor->no_of_new_student = $i->$column7;
                $honor->percent_new_student = $i->$column8;
                $honor->created_by = $user;
                $honor->updated_by = $user;
                try {
                  $honor->save();
  ...
  public function ExportNewPercentExcel(Request $request)
  {
      // select ข้อมูลที่ต้องการ export
      $query = "SELECT *
        FROM new_student_report
      ...
      // ระบุชื่อไฟล์
      $filename = "import_new_student(%)_template";
      $x = Excel::create($filename, function($excel) use($items, $filename) {
      $excel->sheet($filename, function($sheet) use($items) {
